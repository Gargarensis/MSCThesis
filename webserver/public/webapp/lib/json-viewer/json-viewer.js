...
var JSONViewer = (function(document) {
	 ...
	 */
	function walkJSONTree(outputParent, value, maxLvl, colAt, lvl, previousKey) {
		...
		if (typeof realValue === "object" && realValue !== null && !isDate) {
			...
			if (items.length && !isMaxLvl) {
				...
				if (previousKey != undefined && typeof previousKey === "string") {
					ulList.setAttribute("parent-key", previousKey);
				}
				...
				items.forEach(function(key, ind) {
					...
					if (arrCounter != -1) {
						li.setAttribute("data-array-index", arrCounter);
					}

					if (typeof item === "object") {
						// null && date
						if (!item || item instanceof Date) {
							li.appendChild(createCustomTextNode(isArray ? "" : key + ": ", key));
						...
						// array & object
							...
							// empty
							if (!itemLen) {
								li.appendChild(createCustomTextNode(key + ": " + (itemIsArray ? "[]" : "{}"), key));
							}
							else {
								// 1+ items
								...
								walkJSONTree(li, item, maxLvl, colAt, lvl + 1, key);
					...
					// simple values
						...
						if (!isArray) {
							let textNode = createCustomTextNode(key + ": ", key);
	...
	// Riccardo - Create textnodes with custom format
	function createCustomTextNode(text, key) {
		let containerNode = document.createElement("span");
		let textNode = document.createTextNode(text);
		containerNode.setAttribute("need-listener", "true");
